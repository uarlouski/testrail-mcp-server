...
- **How it works**:
  ...
  - **Companion Metadata Sidecar**: Generates a `.md.metadata.json` sidecar file containing structured attributes (`metadataAttributes`) such as case ID, last-updated timestamp (`updated_on`), title, section, priority, references, resolved multi-select tag strings, dropdowns, and checkboxes for vector search filtering, retrieval, and version synchronization.
    - **Why `updated_on` is used for RAG**:
      - **Incremental Syncing**: Knowledge Base ingestion pipelines can compare `updated_on` against stored sidecar metadata (or use TestRail's `updated_after` filter on `get_cases`) to detect modified cases without computing expensive full-text document hashes or diffs, and without an extra `get_history_for_case` call per case.
      - **Stale Vector Eviction**: Invalidate outdated vector database embeddings (e.g. Pinecone, Weaviate, Bedrock Knowledge Bases) as soon as a test case's `updated_on` timestamp changes.
      - **Version Citations**: Enables AI assistants to cite when the exported test steps and requirements were last updated.
- **Export Modes**:
