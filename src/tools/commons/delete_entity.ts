...
const parameters = {
    entity_type: z.enum(["case", "shared_step", "attachment"]).describe("The type of the entity to delete"),
...
export const deleteEntityTool: ToolDefinition<typeof parameters, TestRailClient> = {
    name: "delete_entity",
    mode: "delete",
    description: "Deletes a specified TestRail entity (case, shared_step, or attachment) by its ID.",
    parameters,
    handler: async (args, client) => {
        ...
        switch (entity_type) {
            ...
            case "attachment": {
                await client.deleteAttachment(entity_id);
                return { message: `Attachment ${entity_id} deleted successfully.` };
            }
            default:
